...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    "基本逻辑还是搞清楚了，但是没理解题意，题意需要所有mobile都平衡"
    if is_planet(m):
        return True
    else:
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    "*** YOUR CODE HERE ***"
    "没问题，一开始递归函数调用出了点问题"
    "好像还是有问题啊！！！局部参数和全局参数的问题"
    def helper(s, lokis_replacement):
        if is_leaf(s) and label(s)=='loki':
            s[0]= lokis_replacement
        for item in branches(s):
            helper(item, lokis_replacement)
        return s
    return helper(t, lokis_replacement)
...
def mul_interval(x, y):
    ...
    return interval(min(p1, p2, p3, p4), max(p1, p2, p3, p4)) #返回应该是个interval而不是列表
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y)*upper_bound(y)>0,'zero not be divided'
