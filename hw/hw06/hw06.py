...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        if self.cents+Mint.present_year-self.year>=50:
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    "实现的有点蠢，犯了很多小错误，导致花了很多时间，其实是一个非常简单的递归问题"
    list=[]
    while n>=10:
        last=n%10
        list.append(last)
    ...
    list.append(n%10) 
    i=len(list)-1
    def list_link(i,list):
        if i<0:
            return Link.empty
        else:
            return Link(list[i],list_link(i-1,list))
    return list_link(i,list)
    "妙啊，为什么你总能把循环能解决的问题用递归来解决呢？？？？？"
    """    result = Link.empty
    while n > 0:
...
def deep_map_mut(func, lnk):
    ...
    "*** YOUR CODE HERE ***"
    "独自实现失败"
    "稍微指点即成功，不要return deep_map_mut，直接调用此函数即可"
    "其他逻辑方面无问题"
...
def is_bst(t):
    ...
    def bst_max(t):
        ...
        if t.is_leaf():
            return t.label
        return max(t.label, bst_max(t.branches[-1])) #t.branches[-1]=t.branches[len(branches)],也就是说访问的是最后一个元素
