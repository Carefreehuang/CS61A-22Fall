...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        elif place.ant.is_container and  place.ant.can_contain(self) and self.is_container!=True:
            #逻辑一开始出现了漏洞，但是仍然通过了08的测试，但是在09就出现了问题，已修复
            place.ant=place.ant
            place.ant.store_ant(self) 
            '为什么 place.ant=place.ant.store_ant(self) 这种连在一起的写法会不行呢？？'
        elif place.ant.is_container!=True and self.is_container ==True:
    ...
    def double(self):
        ...
        if self.is_container:
            ...
            if self.ant_contained!=None:
                if self.ant_contained.doubled!=True:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current.is_hive:
            if current.bees and self.lower_bound<=distance<=self.upper_bound:
                return random_bee(current.bees)
            else:
                current=current.entrance
                distance+=1
            """if lower_bound==0 and upper_bound==None:
                if current.bees:
                    return random_bee(current.bees)  # REPLACE THIS LINE
                else:
                    current=current.entrance   #你他妈是真的抽象啊，主打就是理解题意
                else:"""
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    """def nearest_bee(self,lower_bound=0,upper_bound=float('inf')):
        return super().nearest_bee(self.lower_bound, self.upper_bound)"""
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.place.bees:
            if self.health >1:
                for bee_ in list(self.place.bees):  
                    bee_.reduce_health(amount)
            else:
                for bee_ in list(self.place.bees):
                    bee_.reduce_health(amount+self.damage)
        super().reduce_health(amount)
        """为什么迭代的时候不能变异，因为迭代是按照索引进行，如果迭代时的变异导致了元素的减少，那么删除的元素的下一个元素就不会被迭代到
         例如my_list = [1, 2, 3, 4, 5]
            for item in my_list:
                if item == 3:
                    my_list.remove(item)
            print(my_list)  # 输出：[1, 2, 4, 5]
        在对3进行操作时，index=2，删除3后，index+=1，但此时因为3被删除，4前移一位，因此此时4的index=2，所以4无法被迭代到
        """
        # END Problem 5
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        # END Problem 8a
#关键理解题意，理解题意的关键又在于那几道解锁题和题目让你执行的具体函数
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.exist_queen==True:
            return None
        else:
            gamestate.exist_queen=True
            return super().construct(gamestate)   #妈的居然是要return！！！！！
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
                #没有输入gamestate参数导致curretn_place为None
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees!=None:
            for bee in list(self.place.bees):
                bee.reduce_health(self.damage)   # Change to True to view in the GUI
...
class SlowThrower(ThrowerAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    # END Problem EC
    "自己没能实现，不知道如何在不修改BEE的action情况下来控制bee"
    "定义一个新的action函数，若被投掷到，则直接将新的action替换掉原来的bee.action"
    def throw_at(self, target):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not current_place.is_hive:
            ...
            if current_place.ant !=None :
                if current_place==self.place and current_place.ant.name=='Laser':
                    ''''''
                else:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        if self.damage-0.0625*self.insects_shot -distance*0.25>=0:
