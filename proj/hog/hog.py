...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls>=1:
        ...
        if dice_num!=1:
            ...
            flag=True                    #中间break会导致num_rolls非正确的变化，从而导致下次位置不对
        num_rolls-=1
    if(flag):
        roll_sum=1
    return roll_sum
...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    assert opponent_score>=0 , 'opponent_score do not negtive.'
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls==0:
        ...
        "score=roll_dice(num_rolls, six_sided) "
        "因为dice可能是six_sided,four_sided,make_test_dice,本题中大部分都是使用make_test_dice,six_sided只是默认情况。" 
...
# BEGIN PROBLEM 4
"*** YOUR CODE HERE ***"
"需要寻求更好的实现方法，因为下面两个函数有重叠部分，如何合理调用很关键"
...
def play(strategy0, strategy1, update,
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    "为什么把函数strategy放入update内部,outcome就没有内容?"
...
def is_always_roll(strategy, goal=GOAL):
    ...
    roll=strategy(0,0)
    while i<=goal:
        j=0                #每次的外层循环j都要从0-goal所以j=0定义在外层循环之中
        while j<=goal:
...
def make_averaged(original_function, total_samples=1000):
    "调用original_function total_samples次, return = (original_function的结果之和 / total_samples)"
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    "使用高阶函数可以使用*args,从而适用于所有函数作为original_fuction."
    def average(*args):
        sum=0
        i=0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while i<=10:
        ...
        if current_score>max:
            max=current_score  #忘记改变最大值了，还是得多DEBUG打印啊
            max_i=i
        i=i+1
    return max_i
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    if current_score-score>=threshold:   #12只是默认值，正常应该输入threshold，与之前的six_sided一样的错误
        num_rolls=0
    return num_rolls # Remove this line once implemented.
    # END PROBLEM 11
