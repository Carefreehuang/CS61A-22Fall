...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        "不知道实现 if 求值判断 else 直接返回本来值"
        "其实scheme_eval以及进行了表达式还是单个值的判断，因为没有rest.first所以scheme_eval得到pair"
        "然后进行表达式的计算，所以导致：int not call "
...
def do_lambda_form(expressions, env):
    ...
    current_lambda=LambdaProcedure(formals,body,env)
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    "(and) ; 返回 #t，因为没有参数传递给 and，按惯例认为是真"
    current_expr=expressions
    if current_expr==nil:
        return True
    while current_expr.rest!=nil:
        if is_scheme_false(scheme_eval(current_expr.first,env)):
            return False 
        else:
            current_expr=current_expr.rest
    return scheme_eval(current_expr.first,env)
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    "(or) ; 返回 #f，因为没有参数传递给 or，按惯例认为是假"
    current_expr=expressions
    if current_expr==nil:
        return False
    while current_expr.rest!=nil:
        if is_scheme_true(scheme_eval(current_expr.first,env)):
            print("DEBUG",scheme_eval(current_expr.first,env))
            return scheme_eval(current_expr.first,env) 
        else:
            current_expr=current_expr.rest
    return scheme_eval(current_expr.first,env)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            "是真的离谱，为什么scheme_eval需要的是clause.rest.first"
            "而eval_all需要的是clause.rest"
